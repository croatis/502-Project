    ...
    def run(self):
        ...
        while traci.simulation.getMinExpectedNumber() > 0 and traci.simulation.getTime() < self.maxSimulationTime:
            ...
            if step % 5 == 0:  
                ...
                for tl in trafficLights:
                    applied = False  # Variable for user defined rules   
                    ...
                    if self.assignGreenPhaseToSingleWaitingPhase_UDRule:
                        ...
                        if applied is True:
                            print("Applied single lane waiting rule. Skipping the rest of these whores.")
                    ...
                    if self.maxGreenAndYellow_UDRule:
                        ...
                        if applied is True:
                            print("Applied green/yellow rule. Skipping the rest of these whores.")
                            continue

                    if self.maxRedPhaseTime_UDRule:
                        applied = self.checkMaxRedPhaseTimeRule(tl)
                        if applied is True:
                            print("Applied the red rule. Skipping the rest of these whores.")
                            continue    
